...
async def create_league(
    ...
    already_exists = (
        await db.execute(
            select(models.League).where(
                models.League.name == league.name, models.League.user_id == user.id
            )
        )
    ).scalar_one_or_none()
    ...
    if already_exists:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="League already exists"
        )
...
async def get_leagues(
    ...
    leagues = (
        (
            await db.execute(
                select(models.League).where(models.League.user_id == user.id)
            )
        )
        .scalars()
        .all()
...
async def update_league_info(
    ...
    league_to_update = (
        await db.execute(
            select(models.League).where(
                models.League.id == updated_league.id, models.League.user_id == user.id
            )
        )
    ).scalar_one_or_none()
    ...
    if not league_to_update:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="League not found to update"
        )
...
async def delete_league(
    ...
    league_to_delete = (
        await db.execute(
            select(models.League).where(
                models.League.user_id == user.id, models.League.id == id
            )
        )
    ).scalar_one_or_none()
