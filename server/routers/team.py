...
async def create_team(
    ...
    already_exists = (
        await db.execute(
            select(models.Team).where(
                models.Team.name == team.name, models.Team.league_id == league.id
            )
        )
    ).scalar_one_or_none()
    ...
    db_team = models.Team(
        user_id=league.user_id, league_id=league.id, **team.model_dump()
    )
...
async def get_league_teams(
    ...
    teams = (
        (
            await db.execute(
                select(models.Team).where(models.Team.league_id == league.id)
            )
        )
        .scalars()
        .all()
...
async def update_team_info(
    ...
    team_to_update = (
        await db.execute(
            select(models.Team).where(
                models.Team.league_id == league.id, models.Team.id == updated_team.id
            )
        )
    ).scalar_one_or_none()
    ...
    if not team_to_update:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Team not found to update"
        )
...
async def delete_team(
    ...
    team_to_delete = (
        await db.execute(
            select(models.Team).where(
                models.Team.league_id == league.id, models.Team.id == id
            )
        )
    ).scalar_one_or_none()
