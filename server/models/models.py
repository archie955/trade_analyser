...
class User(Base):
    ...
    leagues: Mapped[list["League"]] = relationship(
        "League", back_populates="owner", cascade="all, delete-orphan"
    )
...
class Team(Base):
    ...
    players: Mapped[list["Player"]] = relationship(
        "Player", secondary="team_players", back_populates="teams"
    )
...
class Player(Base):
    ...
    name: Mapped[str] = mapped_column(String(100), nullable=False)
    team: Mapped[Teams] = mapped_column(teamsdt, nullable=False)
    position: Mapped[Positions] = mapped_column(positionsdt, nullable=False)
    points_ppr: Mapped[float] = mapped_column(
        DECIMAL(10, 3), nullable=False, server_default="0.0"
    )
    ...
    teams: Mapped[list["Team"]] = relationship(
        "Team", secondary="team_players", back_populates="players"
    )
