async def test_create_team(auth_client_leagues):
    response = await auth_client_leagues.post(
        "/leagues/1/teams", json={"name": "team_1"}
    )
...
async def test_no_auth_create_team(auth_client_leagues):
    response = await auth_client_leagues.noauth_post(
        "/leagues/1/teams", json={"name": "testteamnoauth"}
    )
...
async def test_create_team_wrong_payload(auth_client_leagues):
    response = await auth_client_leagues.post(
        "/leagues/1/teams", json={"wrong": "testteam"}
    )
...
async def test_create_team_same_name(auth_client_teams):
    response = await auth_client_teams.post(
        "/leagues/1/teams", json={"name": "Trade Team"}
    )
...
async def test_get_teams(auth_client_teams):
    response = await auth_client_teams.get("/leagues/1/teams")
...
async def test_get_teams_no_auth(auth_client_teams):
    response = await auth_client_teams.noauth_get("/leagues/1/teams")

    assert response.status_code == 401


async def test_get_no_teams(auth_client_leagues):
    response = await auth_client_leagues.get("/leagues/1/teams")
...
async def test_update_team(auth_client_teams):
    team_to_update = (await auth_client_teams.get("/leagues/1/teams")).json()["teams"][
        0
    ]

    updated = {"id": team_to_update["id"], "name": "newname"}

    response = await auth_client_teams.put("/leagues/1/teams", json=updated)
...
async def test_update_team_not_auth(auth_client_teams):
    ...
    response = await auth_client_teams.noauth_put("/leagues/1/teams", json=updated)

    assert response.status_code == 401


async def test_delete_team(auth_client_teams):
    teams = (await auth_client_teams.get("/leagues/1/teams")).json()["teams"]

    team_to_delete = teams[0]

    id = team_to_delete["id"]

    response = await auth_client_teams.delete(f"/leagues/1/teams/{id}")

    assert response.status_code == 204

    updated_teams = (await auth_client_teams.get("/leagues/1/teams")).json()["teams"]
...
async def test_delete_team_no_auth(auth_client_teams):
    ...
    response = await auth_client_teams.noauth_delete(f"/leagues/1/teams/{id}")
...
async def test_delete_team_wrong_id(auth_client_teams):
    ...
    response = await auth_client_teams.delete("/leagues/1/teams/9999")
