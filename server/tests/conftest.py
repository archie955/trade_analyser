...
class AuthClient:
    ...
    async def seed_teams(self) -> None:
        team1 = Team(id=1, name="Trade Team", user_id=1, league_id=1)

        team2 = Team(id=2, name="Alt Team", user_id=1, league_id=1)
    ...
    async def seed_players_teams(self, team1_id, team2_id) -> None:
        for p in data["team1"]:
            ...
            self.db.add(TeamPlayer(team_id=team1_id, league_id=1, player_id=player.id))
        ...
        for p in data["team2"]:
            ...
            self.db.add(TeamPlayer(team_id=team2_id, league_id=1, player_id=player.id))
...
class Helpers:
    ...
    async def full_login(client: AsyncClient) -> dict:
        ...
        user_payload = await client.post("/users", json=user)
